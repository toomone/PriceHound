<script lang="ts">
	...
	let negotiatedPriceText = negotiatedPrice != null ? String(negotiatedPrice) : '';
	$: if (negotiatedPrice != null && !negotiatedPriceFocused) negotiatedPriceText = String(negotiatedPrice);
	let negotiatedPriceFocused = false;

	function handleNegotiatedPriceInput() {
		const normalized = negotiatedPriceText.replace(/,/g, '.');
		const parsed = parseFloat(normalized);
		negotiatedPrice = isNaN(parsed) ? null : Math.round(parsed * 100) / 100;
	}

	function handleNegotiatedPriceBlur() {
		negotiatedPriceFocused = false;
		handleNegotiatedPriceInput();
		if (negotiatedPrice != null) negotiatedPriceText = String(negotiatedPrice);
		dispatch('update', { product: selectedProduct, quantity, negotiatedPrice });
	}
					...
					<div class="mt-2 flex items-center gap-2 flex-wrap">
						...
						{#if !isPercentageBased}
							<Tooltip.Root>
								...
								<Tooltip.Content>Specify a custom negotiated unit price (annual billing)</Tooltip.Content>
							</Tooltip.Root>
						{/if}
						<!-- Negotiated price input (when enabled) -->
						{#if showNegotiatedInput && !isPercentageBased}
							<div class="flex items-center gap-1" transition:fly={{ x: -8, duration: 150 }}>
								<span class="text-xs text-amber-600 dark:text-amber-400">$</span>
								<input
									type="text"
									inputmode="decimal"
									bind:value={negotiatedPriceText}
									placeholder={annualPrice.toFixed(2)}
									class="w-24 h-6 px-1.5 text-xs rounded border border-amber-300 dark:border-amber-600 bg-amber-50 dark:bg-amber-950 text-amber-800 dark:text-amber-200 focus:ring-1 focus:ring-amber-500 focus:outline-none"
									on:focus={() => negotiatedPriceFocused = true}
									on:blur={handleNegotiatedPriceBlur}
									on:keydown={(e) => e.key === 'Enter' && e.currentTarget.blur()}
								/>
								<span class="text-[10px] text-amber-600/70 dark:text-amber-400/70">/unit</span>
							</div>
						{/if}
						<!-- Volume breakdown button -->
						{#if !hasBreakdown && !isAllotment}
							<Tooltip.Root>
								<Tooltip.Trigger asChild let:builder>
									...
									>
										<svg class="h-2.5 w-2.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18M3 12h12M3 18h8" /></svg>
										breakdown by env, team...
									</button>
								</Tooltip.Trigger>
								<Tooltip.Content>Split quantity into detail lines per environment, team, or any label</Tooltip.Content>
							</Tooltip.Root>
						{:else if hasBreakdown && !showBreakdown && !isAllotment}
							<Tooltip.Root>
								<Tooltip.Trigger asChild let:builder>
									<button
										use:builder.action
								...
								<Tooltip.Content>Show the {quantityBreakdown.length} volume breakdown lines</Tooltip.Content>
							</Tooltip.Root>
						{/if}
					</div>
				{/if}
			...
			<div class="mt-3 pt-3 border-t border-border/30" transition:slide={{ duration: 150 }}>
				<div class="space-y-1">
					{#each quantityBreakdown as bl (bl.id)}
						<div class="flex items-center gap-2 group/bl pl-4" transition:slide={{ duration: 120 }}>
							<button
								type="button"
