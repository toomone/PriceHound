<script lang="ts">
	...
	function tierUnitPrice(item: import('$lib/api').QuoteLineItem): number {
		if (selectedTier === 'annual') return item.unit_price_annually ?? item.unit_price;
		if (selectedTier === 'monthly') return item.unit_price_monthly ?? item.unit_price;
		return item.unit_price_on_demand ?? item.unit_price;
	}

	function tierTotalPrice(item: import('$lib/api').QuoteLineItem): number {
		if (selectedTier === 'annual') return item.total_price_annually ?? item.total_price;
		if (selectedTier === 'monthly') return item.total_price_monthly ?? item.total_price;
		return item.total_price_on_demand ?? item.total_price;
	}
</script>
							...
							{#each quote.items as item, index}
								...
								>
									<td class="py-4">
										...
										{#if item.negotiated_price && item.negotiated_price > 0 && selectedTier === 'annual'}
								...
								<td class="py-4 text-right font-mono {item.negotiated_price && item.negotiated_price > 0 && selectedTier === 'annual' ? 'text-amber-600 dark:text-amber-400' : ''}">{formatCurrency(tierUnitPrice(item))}</td>
								<td class="py-4 text-right font-mono font-semibold text-datadog-purple">
									{formatCurrency(tierTotalPrice(item))}
								</td>
								</tr>
